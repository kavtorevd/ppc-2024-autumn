...
bool testTaskSequential::validation() {
  if (taskData->inputs_count.empty() || taskData->outputs_count.empty() || taskData->inputs[0] == nullptr ||
...
bool testTaskSequential::run() {
  ...
  std::cout << "Run successful: most different neighbors are (" << result_.first << ", " << result_.second
            << ") with difference " << max_diff << std::endl;
...
bool testTaskSequential::post_processing() {
  ...
  output[0] = result_.first;
  output[1] = result_.second;

  std::cout << "Post-processing successful: result stored in outputs [" << output[0] << ", " << output[1] << "]."
            << std::endl;

  return true;
}

void testTaskSequential::set_data(const std::vector<int>& data) { data_ = data; }

std::pair<int, int> testTaskSequential::get_most_different_neighbors() const { return result_; }
